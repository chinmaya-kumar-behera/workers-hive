          ...
          >
            <div className="w-full lg:w-1/2 space-y-5">
              ...
              <div className="pb-10 space-y-4 text-center">
                ...
                <div className="w-full h-[135px] flex gap-2 lg:gap-4 items-center justify-center overflow-x-auto no-scrollbar">
                  {subcategories
                    ? subcategories
                        .slice(window.innerWidth >= 1024 ? 0:2)
                        .map((value) => (
                          <ShowCard key={value._id} data={value} />
                        ))
                    : Array.from(
                        { length: window.innerWidth >= 1024 ? 4 : 3 },
                        (_, index) => index + 1
                      ).map((value, index) => (
                        <div
                          key={index}
                          className="max-w-[60px] lg:max-w-[90px] max-h-[60px] lg:max-h-[90px] w-[60px] lg:w-[90px] h-[60px] lg:h-[90px] rounded-full"
                        >
                          <Skeleton
                            height="100%"
                            width="100%"
